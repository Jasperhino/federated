...
from utils.models import emnist_cvae_models


def configure_training(task_spec: training_specs.TaskSpec) -> training_specs.RunnerSpec:
    """Configures training for the EMNIST conditional variational autoencoder task.
    ...
    model_builder = emnist_cvae_models.create_cvae_model
    loss_builder = functools.partial(tf.keras.losses.MeanSquaredError, reduction=tf.keras.losses.Reduction.SUM)
    ...
    def client_sampling_fn(x):
        return list(client_ids_fn(x))
